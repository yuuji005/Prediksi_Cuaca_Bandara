...
try:
    ...
    # Load ANN TFLite
    try:
        ann_interpreter = tflite.Interpreter(model_path='models/ann_model.tflite')
        ann_interpreter.allocate_tensors()
        models['ann'] = ann_interpreter
    except Exception as e:
        print("Error loading ANN TFLite:", e)
        models['ann'] = None

    # Load LSTM TFLite
    try:
        lstm_interpreter = tflite.Interpreter(model_path='models/lstm_model.tflite')
        lstm_interpreter.allocate_tensors()
        models['lstm'] = lstm_interpreter
    except Exception as e:
        print("Error loading LSTM TFLite:", e)
        models['lstm'] = None

    models['bp'] = None # Fallback for now
...
def get_features_for_date(target_date, model_choice='lr'):
    ...
    if model_choice == 'lr':
        ...
        if models['ann'] is None:
            return None, "Model ANN tidak tersedia"
        interp = models['ann']
        interp.set_tensor(interp.get_input_details()[0]['index'], features_scaled.astype(np.float32))
        interp.invoke()
        pred = interp.get_tensor(interp.get_output_details()[0]['index'])[0][0]
    elif model_choice == 'lstm':
        if models['lstm'] is None:
            return None, "Model LSTM tidak didukung di Vercel (butuh Flex Delegate)"
        ...
        seq_input = seq_scaled.reshape(1, 3, 4).astype(np.float32)
        interp = models['lstm']
        interp.set_tensor(interp.get_input_details()[0]['index'], seq_input)
        interp.invoke()
        pred = interp.get_tensor(interp.get_output_details()[0]['index'])[0][0]
    elif model_choice == 'bp':
        return None, "Model Backprop tidak tersedia di Vercel"
    else:
